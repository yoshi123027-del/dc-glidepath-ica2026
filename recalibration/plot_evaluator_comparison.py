"""Plot saved, unsmoothed CDFs and weighted glide paths (no new evaluation)."""
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--directory',default='results/recalibration_v8/fine');ap.add_argument('--out');a=ap.parse_args();d=Path(a.directory);o=Path(a.out) if a.out else d;o.mkdir(parents=True,exist_ok=True)
 z=np.load(d/'evaluator_comparison.npz');mc=np.load(d/'independent_mc.npz');x=z['grid'];names=z['names'];colors=['#0072B2','#D55E00','#009E73','#CC79A7','#666666']
 ...
 for kind in ['cdf','glidepath']:
  ...
  for ext in ['png','pdf']:fig.savefig(o/f'fig_evaluator_{kind}_comparison.{ext}',dpi=220)
  plt.close(fig)
