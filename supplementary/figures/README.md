...
上段では、共通平均較正後の終端富分布を密度で比較しています。PCMVはターゲット近傍への集中が強く、分布中央の鋭い山として現れます。一方、dTCMVは分布が広く、右裾への参加を残す形です。下段の同色の横線は5%–95%分位区間、丸印は中央値、白抜き菱形は平均値を示します。各戦略の平均値は較正によりほぼ一致しますが、中央値と分位区間は異なり、分布の形状差を直接確認できます。右端の小さな山には上端格子への確率質量が含まれるため、裾の絶対水準は境界診断と併せて解釈する必要があります。

プレゼン用のCP除外版: [PNG](./fig_four_strategies_terminal_density_D0_N480.png) / [SVG](./fig_four_strategies_terminal_density_D0_N480.svg)
