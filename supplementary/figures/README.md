...
以下の5図は、低残高領域を細かくした格子を用いる年80分割のスクリーニング分析です。結果の方向性と頑健性を確認するための補足資料であり、包括的な格子収束証明ではありません。

### 3.1 DB給付 \(D_T\) に対する感応度

![DB給付に対する感応度](./fig_D_sensitivity_glidepaths_N80.png)

PCMV、DOMVおよびcTCMVでは、確定的なDB給付を加えてもDC部分の最適グライドパスはほぼ重なります。これらの問題では、確定的給付が目的関数の定数項または平行移動として作用するためです。これに対し、総年金富依存のdTCMVではDB給付が有効なリスク回避度に影響し、グライドパスの水準とU字形状が変化します。

### 3.2 安全利子率 \(r\) に対する感応度

![安全利子率に対する感応度](./fig_r_sensitivity_glidepaths_N80.png)

基準値は \(r=0.015\) です。安全利子率が高いほど、安全資産のみでも将来富を積み上げやすくなるため、PCMV、DOMVおよびcTCMVのリスク資産比率は全般に低下します。dTCMVでも中間期間のリスク比率が低下しますが、退職前の再上昇を含む状態依存形状は残ります。

### 3.3 リスク資産期待収益率 \(\mu\) に対する感応度

![期待収益率に対する感応度](./fig_mu_sensitivity_glidepaths_N80.png)

基準値は \(\mu=0.055\) です。期待超過収益が高くなるほど、PCMV、DOMVおよびcTCMVではリスク資産を保有する便益が高まり、リスク比率が上昇します。dTCMVでも水準は変化しますが、低残高時の上限制約と総年金富依存のリスク回避度が同時に作用するため、応答は単純な平行移動にはなりません。

### 3.4 リスク資産ボラティリティ \(\sigma\) に対する感応度

![ボラティリティに対する感応度](./fig_sigma_sensitivity_glidepaths_N80.png)

基準値は \(\sigma=0.18\) です。ボラティリティの上昇は分散費用を高めるため、各戦略のリスク資産比率をおおむね低下させます。dTCMVでは中間期間の谷と退職前の再上昇が残り、パラメータを変えても状態依存的な形状が維持されることが確認できます。

### 3.5 拠出プロファイルに対する感応度

![拠出プロファイルに対する感応度](./fig_contrib_profile_sensitivity_glidepaths_N80.png)

総拠出額を同一に保ち、一定拠出、線形増加および二次的増加を比較しています。拠出を後半へ寄せるほど、加入初期の人的資本、すなわち将来拠出の現在価値が大きくなるため、初期から中期のリスク資産比率が高くなります。総拠出額だけでなく拠出の時期も最適グライドパスを左右することを示しています。
